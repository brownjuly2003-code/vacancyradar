...
The collection runner does `ingest hh → publish slim`. If the ingest step
starts failing (network blip, hh.ru change, crash) but publish slim still
runs, it would happily rebuild days-old data that the HF mirror step then
republishes, and the storefront would surface stale vacancies as fresh.
...
def patched_publish(monkeypatch, tmp_path):
    """chdir to tmp + stub write_slim_active; returns the list of write calls."""
...
def test_publish_slim_warns_when_last_seen_old_default(patched_publish, monkeypatch, capsys):
    ...
    assert exit_code == 0
    assert len(write_calls) == 1  # default mode still writes the artifact
...
def test_publish_slim_strict_exits_when_last_seen_old(patched_publish, monkeypatch, capsys):
    ...
    assert exit_code == 4
    # Strict failure happens BEFORE the artifact write — a stale parquet must
    # not replace the previous-good one that the HF mirror would republish.
